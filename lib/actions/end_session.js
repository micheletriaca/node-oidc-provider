  ...
  init: [
    ...
    async function endSessionChecks(ctx, next) {
      ...
      if (params.id_token_hint) {
        ...
        const { payload: { aud: clientId, azp } } = ctx.oidc.entities.IdTokenHint;

        if (params.client_id && params.client_id !== (azp || clientId)) {
          throw new InvalidRequest('client_id does not match the provided id_token_hint');
        }
        client = await ctx.oidc.provider.Client.find(azp || clientId);
        if (!client) {
